...
const ProjectImageModal = ({ open, image }) => {
  const handleClose = () => {
    open(false);
  };
  return (
    <motion.div className="absolute top-0 left-0 w-full h-screen ">
      <motion.div
        ...
        dragElastic={0.2}
        dragMomentum={false}
        className="absolute  top-[100px] left-[200px] w-[450px] h-[200px] rounded-lg bg-[#1E1E1E] "
      >
        <Dots handleClose={handleClose} nameWindow="Documentation" />
        <div className=" text-white w-[600px] h-full mt-[55px] ">
          <Image src={image} alt="Project Image" width={450} height={350} />
        </div>
      </motion.div>
    </motion.div>
  );
